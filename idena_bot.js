...
let idenaAddress = []
    // 判断环境变量里面是否有京东ck
if (process.env.IDENA_ADDRESS) {
    if (process.env.IDENA_ADDRESS.indexOf('&') > -1) {
        idenaAddress = process.env.IDENA_ADDRESS.split('&');
    } else if (process.env.IDENA_ADDRESS.indexOf('\n') > -1) {
        idenaAddress = process.env.IDENA_ADDRESS.split('\n');
    } else {
        idenaAddress = [process.env.IDENA_ADDRESS];
    }
}
...
async function idenaCheckOnline() {
        ...
        $.get(options, (err, resp, data) => {
            try {
                if (err) {
                    ...
                    data = JSON.parse(data);
                    if (data.result) {
                        const balance = Number.parseFloat(data.result.balance)
                        const stake = Number.parseFloat(data.result.stake)
